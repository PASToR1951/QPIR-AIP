  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        ...
        onClick={onClose}
      />
      <div className="relative bg-white dark:bg-dark-surface border border-slate-200 dark:border-dark-border rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden">
        ...
        {/* PIR Summary */}
        <div className="px-6 pt-5 pb-4 bg-slate-50 dark:bg-dark-base border-b border-slate-200 dark:border-dark-border space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">School</p>
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate">{pir.school}</p>
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{pir.program}</p>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Quarter</p>
              <p className="text-sm text-slate-600 dark:text-slate-300">{pir.quarter ?? '—'}</p>
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Submitted By</p>
              <p className="text-sm text-slate-600 dark:text-slate-300 truncate">{pir.submittedBy ?? '—'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-5 flex-1">
          ...
          {fetchLoading ? (
            <div className="flex items-center justify-center h-28">
              <div className="w-5 h-5 rounded-full border-2 border-slate-300 dark:border-slate-600 border-t-accent animate-spin" />
            </div>
          ) : (
            <textarea
              value={remarks}
              ...
              className="w-full px-3 py-2.5 text-sm bg-slate-50 dark:bg-dark-base border border-slate-200 dark:border-dark-border rounded-xl resize-none text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-accent transition-colors"
            />
          )}
          {error && <p className="mt-2 text-xs text-red-500 dark:text-red-400">{error}</p>}
          {saved && <p className="mt-2 text-xs text-emerald-600 dark:text-emerald-400 font-bold">Remarks saved successfully.</p>}
        </div>

        {/* Footer */}
        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-200 dark:border-dark-border shrink-0">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-dark-border rounded-xl transition-colors"
          >
            Close
          </button>
          <button
