          ...
          <div className="max-h-52 overflow-y-auto py-1">
            {filtered.length === 0 ? (
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2 ${selected.includes(o.value) ? 'bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-dark-border'}`}
              >
                <span className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${selected.includes(o.value) ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300 dark:border-slate-600'}`}>
