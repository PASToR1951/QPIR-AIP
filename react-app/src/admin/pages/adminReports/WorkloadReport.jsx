...
export function WorkloadReport({ year, quarter }) {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  useEffect(() => {
    setLoading(true); setError(null);
    api.get(`/api/admin/reports/workload?year=${year}&quarter=${quarter}`)
  ...
  const chartData = data.map(p => ({ name: (p.name ?? p.email).split(' ')[0], programs: p.programCount, pirs: p.pirCount }));
    ...
    <div className="space-y-6">
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={chartData} layout="vertical" barCategoryGap="25%">
          ...
          <Bar dataKey="pirs" name={`Q${quarter} PIRs`} fill="#3b82f6" radius={[0, 4, 4, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
