...
function Section({ title, children }) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-dark-border dark:bg-dark-surface">
      <h2 className="mb-4 border-l-4 border-blue-400 pl-3 text-sm font-black uppercase tracking-widest text-slate-700 dark:text-slate-200">
        {title}
      </h2>
      {children}
    </section>
...
export default function FocalPersonReview({ type }) {
  const { id } = useParams();
  ...
  const titleMeta = useMemo(() => {
    if (!document) return '';
    return type === 'aip' ? `FY ${document.year}` : document.quarter;
  }, [document, type]);

  const handleAction = async () => {
    if (!modal) return;
    if (modal === 'return' && !remarks.trim()) {
      setError('Remarks are required when returning a document.');
      return;
    }
  ...
  if (!document) {
    return <div className="p-12 text-center text-sm text-slate-400">Document not found.</div>;
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        <div className="flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-950/40">
          <CheckCircle size={30} className="text-blue-600 dark:text-blue-300" weight="fill" />
        </div>
        <p className="text-base font-bold text-slate-700 dark:text-slate-200">
          {modal === 'recommend' ? `${label} recommended for CES review.` : `${label} returned to school.`}
        </p>
        <button onClick={() => navigate('/division')} className="rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-blue-700">
          Back to Queue
        </button>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <div className="flex items-center justify-between gap-3">
        <button onClick={() => navigate('/division')} className="flex items-center gap-2 text-xs font-bold text-slate-500 transition-colors hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
          <ArrowLeft size={14} />
          Back to Queue
        </button>
        {canAct && (
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setModal('recommend'); setRemarks(''); setError(''); }} className="flex items-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-blue-700">
              <CheckCircle size={13} />
              Recommend
            </button>
          </div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="mb-2 inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2 py-1 text-xs font-black text-slate-600 dark:bg-dark-border dark:text-slate-300">
              {type === 'aip' ? <FileText size={13} /> : <Table size={13} />}
              {label}
            </div>
            <h1 className="text-xl font-black text-slate-800 dark:text-slate-100">{document.program}</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{document.school} · {titleMeta}</p>
          </div>
          <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[10px] font-black uppercase tracking-wide text-amber-700 dark:border-amber-700/60 dark:bg-amber-950/40 dark:text-amber-300">
            {document.status}
          </span>
        </div>
      </div>
      ...
      {type === 'aip' ? (
        ...
        <>
          ...
          <Section title="Activity Reviews">
            <div className="space-y-3">
              {(document.activities ?? []).map(activity => (
                <div key={activity.id} className="rounded-xl border border-slate-100 p-4 text-sm dark:border-dark-border">
                  <p className="font-bold text-slate-700 dark:text-slate-200">{activity.name || 'Untitled Activity'}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl dark:border-dark-border dark:bg-dark-surface">
            <h2 className="mb-1 text-base font-black text-slate-800 dark:text-slate-100">
              {modal === 'recommend' ? `Recommend ${label}` : `Return ${label}`}
            </h2>
            <p className="mb-4 text-xs text-slate-500 dark:text-slate-400">{document.program} · {titleMeta}</p>
            <label className="mb-1.5 block text-xs font-bold text-slate-600 dark:text-slate-300">
              {modal === 'recommend' ? 'Remarks (optional)' : 'Remarks / reason for return'}
            </label>
            <textarea
              rows={4}
              value={remarks}
              onChange={e => setRemarks(e.target.value)}
              className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-blue-300 dark:border-dark-border dark:bg-dark-base dark:text-slate-200 dark:focus:ring-blue-700"
            />
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button onClick={handleAction} disabled={submitting} className={`rounded-xl px-4 py-2 text-xs font-bold text-white transition-colors ${modal === 'recommend' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-amber-600 hover:bg-amber-700'} disabled:opacity-60`}>
                {submitting ? 'Saving...' : modal === 'recommend' ? 'Recommend' : 'Return'}
              </button>
            </div>
          </div>
        </div>
