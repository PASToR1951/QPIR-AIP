...
const ErrorPage = ({ type = "500", title, message, onRetry }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans text-slate-900 group/screen select-none">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#e2e8f0_1px,transparent_1px),linear-gradient(to_bottom,#e2e8f0_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_130%)] pointer-events-none z-[6] opacity-30"></div>
      ...
      <div className="relative z-20 flex flex-col items-center text-center max-w-2xl px-4">
        ...
        {/* Status indicator */}
        <div className="mb-8">
           <div className="text-6xl md:text-7xl font-black text-slate-900 tracking-tighter uppercase bg-clip-text text-transparent bg-gradient-to-r from-slate-900 via-slate-700 to-slate-900">{type} ERROR</div>
        </div>
        ...
        <div className="bg-white/40 backdrop-blur-xl border border-white/40 p-10 md:p-12 rounded-[3.5rem] shadow-2xl shadow-pink-200/20 max-w-xl w-full mb-12 relative overflow-hidden group">
            ...
            <div className="flex justify-center mb-8">
                <div className="p-6 bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 group-hover:scale-110 transition-transform duration-500">
                    {config.icon}
                </div>
            </div>
