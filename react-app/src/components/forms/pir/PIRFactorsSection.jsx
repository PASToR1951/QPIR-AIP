                ...
                <>
                    ...
                    <div className="space-y-6">
                        {FACTOR_TYPES.map((type) => (
                            <div key={type} className="bg-slate-50 dark:bg-dark-base border border-slate-200 dark:border-dark-border rounded-3xl p-6 md:p-8">
                                ...
                                <div className={`grid grid-cols-1 ${showRecommendations ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-6`}>
            ...
            {appMode === 'full' && (
                <>
                    <div className="mb-6 flex items-center gap-3 border-b border-slate-200 dark:border-dark-border pb-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">Facilitating & Hindering Factors</h2>
                        </div>
                    </div>
                    ...
                    <div className="overflow-x-auto pb-2">
                        <div className="border border-slate-200 dark:border-dark-border rounded-2xl bg-white dark:bg-dark-surface shadow-sm overflow-hidden min-w-[800px]">
                            <div className={`grid ${showRecommendations ? 'grid-cols-3' : 'grid-cols-2'} bg-slate-50 dark:bg-dark-base border-b border-slate-200 dark:border-dark-border font-bold text-center text-sm uppercase tracking-wider`}>
                                ...
                                <div className={`p-3 ${showRecommendations ? 'border-r border-slate-200 dark:border-dark-border' : ''} text-rose-700`}>Hindering Factors</div>
                                {showRecommendations && <div className="p-3 text-blue-700">Recommendations</div>}
                            </div>

                            {FACTOR_TYPES.map((type, idx) => (
                                <div key={type} className={`grid ${showRecommendations ? 'grid-cols-3' : 'grid-cols-2'} bg-white dark:bg-dark-surface ${idx !== FACTOR_TYPES.length - 1 ? 'border-b border-slate-200 dark:border-dark-border' : ''}`}>
