...
export default React.memo(function PIRMonitoringEvaluationSection({
    ...
    const renderGapPanel = (title, colorClass, targetValue, accomplishedValue, gapValue, onTargetChange, onAccomplishedChange) => (
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition-colors focus-within:border-blue-300 dark:border-dark-border dark:bg-dark-surface">
        ...
        <div className={`${(appMode === 'full' || currentStep === 3) ? 'block animate-in fade-in slide-in-from-bottom-4 duration-200' : 'hidden'} ${appMode === 'full' ? 'mb-16' : ''}`}>
            ...
            <PhaseActivityEditor
                        ...
                        <div className="flex flex-col gap-6">
                            <div className="grid grid-cols-1 gap-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-dark-border dark:bg-dark-base md:grid-cols-2">
                                {renderGapPanel(
                                    ...
                                    activity.physAcc,
                                    physGap,
                                    (event) => handleActivityChange(activity.id, 'physTarget', event.target.value.replace(/[^0-9.]/g, '')),
                                    (event) => handleActivityChange(activity.id, 'physAcc', event.target.value.replace(/[^0-9.]/g, '')),
                                )}
                                {renderGapPanel(
                                    'Financial Targets',
                                    'bg-emerald-500',
                                    ...
                                    activity.finAcc,
                                    finGap,
                                    (event) => handleActivityChange(activity.id, 'finTarget', event.target.value.replace(/[^0-9.]/g, '')),
                                    (event) => handleActivityChange(activity.id, 'finAcc', event.target.value.replace(/[^0-9.]/g, '')),
                                )}
                            </div>

                            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 dark:border-dark-border dark:bg-dark-surface dark:focus-within:ring-blue-950/20">
                                <label className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 dark:border-dark-border dark:bg-dark-surface dark:focus-within:ring-blue-950/20">
                                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Actual Tasks Conducted</label>
                                    <TextareaAuto
                                        placeholder="What tasks were actually conducted?"
                                        className="min-h-[40px] w-full bg-transparent text-sm font-medium text-slate-700 outline-none dark:text-slate-200"
                                        value={activity.actualTasksConducted}
                                        onChange={(event) => handleActivityChange(activity.id, 'actualTasksConducted', event.target.value)}
                                    />
                                </div>
                                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 dark:border-dark-border dark:bg-dark-surface dark:focus-within:ring-blue-950/20">
                                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Contributory Performance Indicators</label>
                                    <TextareaAuto
                                        placeholder="Indicators this activity contributes to..."
                                        className="min-h-[40px] w-full bg-transparent text-sm font-medium text-slate-700 outline-none dark:text-slate-200"
                                        value={activity.contributoryIndicators}
                                        onChange={(event) => handleActivityChange(activity.id, 'contributoryIndicators', event.target.value)}
                                    />
                                </div>
                                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 dark:border-dark-border dark:bg-dark-surface dark:focus-within:ring-blue-950/20">
                                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">MOVs / Expected Outputs</label>
                                    <TextareaAuto
                                        placeholder="Means of verification and expected outputs..."
                                        className="min-h-[40px] w-full bg-transparent text-sm font-medium text-slate-700 outline-none dark:text-slate-200"
                                        value={activity.movsExpectedOutputs}
                                        onChange={(event) => handleActivityChange(activity.id, 'movsExpectedOutputs', event.target.value)}
                                    />
                                </div>
                                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors focus-within:border-blue-300 focus-within:ring-2 focus-within:ring-blue-50 dark:border-dark-border dark:bg-dark-surface dark:focus-within:ring-blue-950/20">
                                    <label className="mb-2 block text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Adjustments</label>
                                    <TextareaAuto
                                        placeholder="Any adjustments made to the activity..."
                                        className="min-h-[40px] w-full bg-transparent text-sm font-medium text-slate-700 outline-none dark:text-slate-200"
                                        value={activity.adjustments}
                                        onChange={(event) => handleActivityChange(activity.id, 'adjustments', event.target.value)}
                                    />
                                </div>
                            </div>
                        </div>
                ...
                renderMobileCard={(activity, context) => {
                    ...
                    return (
                        <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-dark-border dark:bg-dark-surface">
                            ...
                            {!activity.fromAIP && (
                                <div className="mb-4">
                                    <label className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Activity Name / Description</label>
                                    <TextareaAuto
                                        placeholder="Describe the activity here..."
                                        className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-800 outline-none transition-colors focus:border-blue-300 dark:border-dark-border dark:bg-dark-base dark:text-slate-100"
                                        value={activity.name}
                                        onChange={(event) => handleActivityChange(activity.id, 'name', event.target.value)}
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-dark-border dark:bg-dark-base">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => handleActivityChange(activity.id, 'physTarget', event.target.value.replace(/[^0-9.]/g, ''))}
                                    />
                                </div>
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-dark-border dark:bg-dark-base">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => handleActivityChange(activity.id, 'physAcc', event.target.value.replace(/[^0-9.]/g, ''))}
                                    />
                                </div>
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-dark-border dark:bg-dark-base">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => handleActivityChange(activity.id, 'finTarget', event.target.value.replace(/[^0-9.]/g, ''))}
                                    />
                                </div>
                                <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-dark-border dark:bg-dark-base">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => handleActivityChange(activity.id, 'finAcc', event.target.value.replace(/[^0-9.]/g, ''))}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="mt-4">
                                <label className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Actions to Address Gap</label>
                                <TextareaAuto
                                    placeholder="What steps will be taken?"
                                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 outline-none transition-colors focus:border-blue-300 dark:border-dark-border dark:bg-dark-base dark:text-slate-200"
                                    value={activity.actions}
                                    onChange={(event) => handleActivityChange(activity.id, 'actions', event.target.value)}
                                />
                            </div>
                ...
                desktopTable={{
                    ...
                    header: (
                        <thead>
                            <tr className="select-none border-b border-slate-200 bg-slate-50 text-center dark:border-dark-border dark:bg-dark-base">
                                <th rowSpan="2" className="w-1/5 border-r border-slate-200 p-4 text-xs font-bold uppercase tracking-wider text-slate-600 dark:border-dark-border dark:text-slate-300">Activity Name</th>
                                <th rowSpan="2" className="w-[130px] border-r border-slate-200 p-4 text-xs font-bold uppercase tracking-wider text-slate-600 dark:border-dark-border dark:text-slate-300">Implementation Period</th>
                                <th colSpan="2" className="border-r border-slate-200 p-2 text-xs font-bold uppercase tracking-wider text-slate-600 dark:border-dark-border dark:text-slate-300">Target</th>
                                <th colSpan="2" className="border-r border-slate-200 p-2 text-xs font-bold uppercase tracking-wider text-slate-600 dark:border-dark-border dark:text-slate-300">Accomplishment</th>
                                <th colSpan="2" className="border-r border-slate-200 p-2 text-xs font-bold uppercase tracking-wider text-slate-600 dark:border-dark-border dark:text-slate-300">Gap (%)</th>
                                <th rowSpan="2" className="w-1/5 p-4 text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300">Actions to Address Gap</th>
                                <th rowSpan="2" className="w-14 border-none"></th>
                            </tr>
                            <tr className="select-none border-b border-slate-200 bg-white text-center dark:border-dark-border dark:bg-dark-surface">
                                <th className="border-r border-slate-200 p-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:border-dark-border dark:text-slate-500">Physical</th>
                                <th className="border-r border-slate-200 p-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:border-dark-border dark:text-slate-500">Financial</th>
                                <th className="border-r border-slate-200 p-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:border-dark-border dark:text-slate-500">Physical</th>
                                <th className="border-r border-slate-200 p-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:border-dark-border dark:text-slate-500">Financial</th>
                                <th className="border-r border-slate-200 p-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:border-dark-border dark:text-slate-500">Physical</th>
                                <th className="border-r border-slate-200 p-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 dark:border-dark-border dark:text-slate-500">Financial</th>
                    ...
                    renderRow: (activity) => {
                        ...
                        return (
                            <tr key={activity.id} className="group border-b border-slate-200 transition-colors hover:bg-slate-50 dark:border-dark-border dark:hover:bg-dark-base">
                                <td className="border-r border-slate-200 p-3 align-top dark:border-dark-border">
                                    {activity.fromAIP ? (
                                        <p className="p-1 text-sm font-medium text-slate-700 dark:text-slate-200">{activity.name}</p>
                                    ) : (
                                        <TextareaAuto
                                            placeholder="Describe activity..."
                                            className="w-full rounded-md border border-transparent bg-transparent p-1 font-medium text-slate-700 focus:border-slate-300 focus:bg-white dark:text-slate-200 dark:focus:border-dark-border dark:focus:bg-dark-surface"
                                            value={activity.name}
                                            onChange={(event) => handleActivityChange(activity.id, 'name', event.target.value)}
                                        />
                                    )}
                                </td>
                                <td className="border-r border-slate-200 bg-blue-50/30 p-3 align-top dark:border-dark-border dark:bg-blue-950/20">
                                    {activity.implementation_period ? (
                                ...
                                <td className="h-px border-r border-slate-200 p-1 align-top dark:border-dark-border">
                                    <input type="number" inputMode="decimal" className="min-h-[44px] w-full rounded-md border border-transparent bg-transparent text-center font-mono text-sm font-semibold text-slate-700 outline-none focus:border-slate-300 focus:bg-white dark:text-slate-200 dark:focus:border-dark-border dark:focus:bg-dark-surface" value={activity.physTarget} onChange={(event) => handleActivityChange(activity.id, 'physTarget', event.target.value.replace(/[^0-9.]/g, ''))} />
                                </td>
                                <td className="h-px border-r border-slate-200 p-1 align-top dark:border-dark-border">
                                    <input type="number" inputMode="decimal" className="min-h-[44px] w-full rounded-md border border-transparent bg-transparent text-center font-mono text-sm font-semibold text-slate-700 outline-none focus:border-slate-300 focus:bg-white dark:text-slate-200 dark:focus:border-dark-border dark:focus:bg-dark-surface" value={activity.finTarget} onChange={(event) => handleActivityChange(activity.id, 'finTarget', event.target.value.replace(/[^0-9.]/g, ''))} />
                                </td>
                                <td className="h-px border-r border-slate-200 p-1 align-top dark:border-dark-border">
                                    <input type="number" inputMode="decimal" className="min-h-[44px] w-full rounded-md border border-transparent bg-transparent text-center font-mono text-sm font-semibold text-slate-700 outline-none focus:border-slate-300 focus:bg-white dark:text-slate-200 dark:focus:border-dark-border dark:focus:bg-dark-surface" value={activity.physAcc} onChange={(event) => handleActivityChange(activity.id, 'physAcc', event.target.value.replace(/[^0-9.]/g, ''))} />
                                </td>
                                <td className="h-px border-r border-slate-200 p-1 align-top dark:border-dark-border">
                                    <input type="number" inputMode="decimal" className="min-h-[44px] w-full rounded-md border border-transparent bg-transparent text-center font-mono text-sm font-semibold text-slate-700 outline-none focus:border-slate-300 focus:bg-white dark:text-slate-200 dark:focus:border-dark-border dark:focus:bg-dark-surface" value={activity.finAcc} onChange={(event) => handleActivityChange(activity.id, 'finAcc', event.target.value.replace(/[^0-9.]/g, ''))} />
                                </td>
                                <td className="h-px border-r border-slate-200 bg-slate-50/50 p-1 align-top dark:border-dark-border dark:bg-dark-base/50">
                                    <input type="text" readOnly tabIndex={-1} className="min-h-[44px] w-full bg-transparent text-center font-mono text-sm font-bold outline-none pointer-events-none select-none" style={{ color: physGap < 0 ? '#ef4444' : '#64748b' }} value={`${physGap.toFixed(2)}%`} />
                                </td>
                                <td className="h-px border-r border-slate-200 bg-slate-50/50 p-1 align-top dark:border-dark-border dark:bg-dark-base/50">
                                    <input type="text" readOnly tabIndex={-1} className="min-h-[44px] w-full bg-transparent text-center font-mono text-sm font-bold outline-none pointer-events-none select-none" style={{ color: finGap < 0 ? '#ef4444' : '#64748b' }} value={`${finGap.toFixed(2)}%`} />
                                </td>
                                ...
                                <td className="w-10 p-2 text-center align-middle">
                                    {activities.length > 1 && (
                                        <button
                                            type="button"
                                            onClick={() => handleRemoveActivity(activity.id)}
                                            className="mx-auto flex min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-slate-300 transition-colors hover:bg-red-50 hover:text-red-500 dark:text-slate-600 dark:hover:bg-red-950/30"
                                            title="Delete Row"
                                        >
