...
function resolveQuarterRange(deadline: unknown, quarter: number): PeriodRange {
...
dashboardRoutes.get(
  ...
  asyncHandler(
    ...
    async (c) => {
      ...
      const allAipIds: number[] = userAIPsWithActivities.map((aip: any) =>
        aip.id
      );
