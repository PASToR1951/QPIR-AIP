...
// Pending OAuth users are inactive until they complete this wizard.
onboardingRoutes.use(
  "/*",
  requireAuth("Authentication required", { allowInactivePending: true }),
);
...
const completeProfileHandler = asyncHandler(
  "Unhandled route error",
  ...
  async (c) => {
    ...
    if (tokenUser.role !== "Pending") {
      return c.json({
        error: "Forbidden. User is already active or not pending.",
      }, 403);
    }
    ...
    if (
      user_type === "Division Personnel" && Array.isArray(program_ids) &&
      ...
      if (admins.length > 0) {
        const notifs = await tx.notification.createManyAndReturn({
          data: admins.map((admin) => ({
            ...
            message:
              `${user.name} (${user.email}) self-registered as ${user.role}.`,
...
onboardingRoutes.post("/complete-profile", completeProfileHandler);
onboardingRoutes.post("/complete_profile", completeProfileHandler);
